...
module.exports = function generate(ctx) {
  ...
  // Add Ionic Specific Code
  if (ctx.driver === 'ng2ionic') {
    schema.push({
      template: './shared/storage/storage.ionic.ts',
      output: '/storage/storage.ionic.ts',
      params: {}
    });
  }
  ...
  function buildModuleImports(models, isIo, driver) {
    ...
    switch (driver) {
      case 'ng2web':
        ...
        break;
      case 'ng2ionic':
        imports.push({module: 'StorageIonic', from: './storage/storage.ionic'});
        if (isIo === 'enabled') {
          imports.push({module: 'SocketBrowser', from: './sockets/socket.browser'});
        }
        break;
  ...
  function buildNgModuleImports(models, environment, isIo, driver) {
    ...
    switch (environment) {
      ...
      case 'ionic':
        if (driver === 'ng2ionic') {
          imports.push('internalStorageProvider');
          imports.push('{ provide: SDKStorage, useClass: StorageIonic }');
          if (isIo === 'enabled') {
            imports.push('{ provide: SocketDriver, useClass: SocketBrowser }');
          }
        }
